...
class Soal extends BaseController
{
    ...
    public function showgroupsk() {
        $res = $this->soalmodel->getjawAllJMateri()->getResult();
        $no = 1;
        if (count($res)>0) {
            $ret = "<table id='example2' class='table table-bordered table-hover'>
                    ...
                    <th style='text-align:center;width:100px;'>Nama Group</th>
                    ...
                    foreach ($res as $key) {
                        $ret .= "<tr>
                            ...
                            <td style='text-align:center;'>".$key->sk_group_nm."</td>
                            </tr>";
                    }
    ...
    public function tambahsoallatihan() {
        ...
        $ret = "<div class='card'>
                ...
                <div class='form-group'>
                    ...
                    $allmateri = $this->soalmodel->getMateriSK()->getResult();
                        foreach ($allmateri as $mat) {
                            $materi_id = $mat->materi_id;
                            $ret .= "<label style='margin:0px 10px;' for='materix_${materi_id}'><input value='".$mat->materi_id."' type='radio' id='materix_${materi_id}' name='materix' ".($materi_id==$this->session->materi?'checked':'checked')."/> ".$mat->materi_nm."</label>";
                        }
                    $ret .= "</div>
                    <div class='form-group row'>
    ...
    public function editsoallatihan() {
        $soal_id = $this->request->getPost('soal_id');
        $ressoal = $this->soalmodel->getSoalByid($soal_id)->getResult();
        $clue = "";
        if (count($ressoal)>0) {
