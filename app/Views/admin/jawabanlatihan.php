              ...
              <div class="col-lg-12">
                ...
                <div class="col-lg-10" style="display:inline-block;text-align:center;width:100%;">
                  <?php
                    foreach ($jenis as $k) {
                  ?>
                  <label> <input <?= ($k->jenis_id == $this->session->jenis_filter?"checked":"") ?> onclick="showjawaban('filter')" type="radio" value="<?= $k->jenis_id ?>" id="jenis_filter" name="jenis_filter" style="margin-left:10px;"/> <?= $k->jenis_nm ?> </label>
                  <?php
                    }
                  ?>
                </div>
...
<script>
  ...
  function showjawaban(filter) {
      ...
      if (jenis == undefined && filter == "filter") {
        ...
        $.ajax({
          url: "<?= base_url('jawabanlatihan/showjawaban') ?>",
          type: "post",
          data: {
            "filter" : filter,
            "jenis": jenis,
  ...
  function tambahjawaban() {
    $.ajax({
        url: "<?= base_url('jawabanlatihan/tambahjawaban') ?>",
        success: function(data) {
  ...
  function editjawaban(jawaban_id,jenis) {
    $.ajax({
        url: "<?= base_url('jawabanlatihan/editjawaban') ?>",
        type: "post",
        data: {
  ...
  function simpanjawaban() {
      ...
      var jenis = $("#soal_id").find(':selected').data('jenis');
      ...
      $.ajax({
        url: "<?= base_url('jawabanlatihan/simpanjawaban') ?>",
        type: "post",
        data: formdata,
        contentType: false,
        processData: false,
  ...
  function updatejawaban(jawaban_id) {
    ...
    $.ajax({
      url: "<?= base_url('jawabanlatihan/updatejawaban') ?>",
      type: "post",
      data: formdata,
      contentType: false,
      processData: false,
...
function hapusjawaban(jawaban_id) {
  ...
  if (confirm(text) == true) {
    $.ajax({
      url: "<?= base_url('jawabanlatihan/hapusjawaban') ?>",
      type: "post",
      data: {
        "jawaban_id": jawaban_id
      },
